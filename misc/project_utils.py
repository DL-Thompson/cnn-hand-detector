...
def play_images_from_datagen(gen_iterator, model=None, interval=100, threshold=0.5, num_to_iter=1):
    ...
    for _ in range(gen_iterator.n):
        ...
        img_copy = x[0].copy()
        img_copy *= 255
        img_copy = cv2.resize(img_copy, (960, 720))
        anno = y[0].copy()
        cv2.rectangle(img_copy, (int(anno[2]), int(anno[1])), (int(anno[4]), int(anno[3])), (255, 0, 0), 2)
        if model:
            prediction = model.predict(x)
            output = prediction[0]
            confidence = output[0]
            if confidence >= threshold:
                cv2.rectangle(img_copy, (int(output[2]), int(output[1])), (int(output[4]), int(output[3])), (0, 255, 0), 2)
            if len(anno) > 0:
                iou_value = get_iou_value(anno, prediction)
                font_scale = 1.5
                spacing = 50
                line_size = 2
                cv2.putText(img_copy, 'IoU: {:.2f}'.format(iou_value), (5, 45), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
                cv2.putText(img_copy, 'Conf: {:.2f}'.format(output[0]), (5, 45 + spacing), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
                cv2.putText(img_copy, 'Y1: {:.0f}'.format(output[1]), (5, 45 + 2 * spacing), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
                cv2.putText(img_copy, 'X1: {:.0f}'.format(output[2]), (5, 45 + 3 * spacing), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
                cv2.putText(img_copy, 'Y2: {:.0f}'.format(output[3]), (5, 45 + 4 * spacing), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
                cv2.putText(img_copy, 'X2: {:.0f}'.format(output[4]), (5, 45 + 5 * spacing), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), line_size, cv2.LINE_AA)
...
def play_images_from_video(video_path, model, interval=100, scale_up=False, threshold=0.5):
    def scale_u(a):
        a[1] *= ORIGINAL_IMAGE_HEIGHT
        a[2] *= ORIGINAL_IMAGE_WIDTH
        a[3] *= ORIGINAL_IMAGE_HEIGHT
        a[4] *= ORIGINAL_IMAGE_WIDTH
    fig, ax = plt.subplots(figsize=(5, 5))
    ...
    while(cap.isOpened()):
        ...
        if ret:
            ...
            if confidence >= threshold:
                if scale_up:
                    scale_u(output)
                cv2.rectangle(frame, (int(output[2]), int(output[1])), (int(output[4]), int(output[3])), (0, 255, 0), 2)
            cv2.putText(frame, 'Conf: {:.1f}'.format(output[0]), (2, 35), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'Y1: {:.1f}'.format(output[1]), (2, 75), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'X1: {:.1f}'.format(output[2]), (2, 125), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'Y2: {:.1f}'.format(output[3]), (2, 175), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(frame, 'X2: {:.1f}'.format(output[4]), (2, 225), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
...
def create_augmented_images(directory_list, json_list, save_directory, num_augmented, bounding_percentage=0.25):
    ...
    def get_annotation(entry, confidence):
        width = entry['annotations'][0]['coordinates']['width']
        height = entry['annotations'][0]['coordinates']['height']
        x = entry['annotations'][0]['coordinates']['x'] - width / 2
        y = entry['annotations'][0]['coordinates']['y'] - height / 2
        ...
        for i, directory in enumerate(directory_list):
            ...
            if json_list[i] is not None:
                ...
                for entry in json_data:
                    ...
                    image = cv2.imread(directory + file_name)
                    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    annotation = get_annotation(entry, 1.0)
...
def plot_history(history):
    ...
    ax[1].set_title('Training IOU Metric')
    ax[1].set_xlabel('Epoch')
    ax[1].set_ylabel('IOU')
    ax[1].set_ylim([0, 1])
    ax[1].plot(history.history['iou_metric'], label='Training')
    ax[1].plot(history.history['val_iou_metric'], label='Validation')
    ax[1].legend()
...
def write_results(file_path, model, history, n_training, n_validation, pos_neg_counts=None):
    ...
    df_dict['iou_metric'] = history.history['iou_metric']
    df_dict['val_iou_metric'] = history.history['val_iou_metric']        
...
def plot_results(results):
    ...
    if 'n_negative' in results and 'n_positive' in results:
        print('Number of Positive Examples: ', results['n_positive'])
        print('Number of Negative Examples: ', results['n_negative'])
    print('Best IOU Training: ', max(results['history']['iou_metric']))
    print('Best IOU Validation: ', max(results['history']['val_iou_metric']))
