...
def play_images_from_sequence_datagen(generator, model=None, interval=100, threshold=0.5, num_to_iter=1, scaled_targets=False):
    fig, ax = plt.subplots(figsize=(5, 5))
    animation_images = []
    for i in range(0, generator.get_length(), num_to_iter):
        ...
        if model is not None and is_model_multi(model):
            ...
            if scaled_targets:
                pred_y1 *= ORIGINAL_IMAGE_HEIGHT
                pred_x1 *= ORIGINAL_IMAGE_WIDTH
                pred_y2 *= ORIGINAL_IMAGE_HEIGHT
                pred_x2 *= ORIGINAL_IMAGE_WIDTH
            if conf_pred >= threshold:
                cv2.rectangle(image_copy, (int(pred_x1), int(pred_y1)), (int(pred_x2), int(pred_y2)), (0, 255, 0), 2)
            iou_value = get_iou_value_multi(annotation, prediction[1][0])
            cv2.putText(image_copy, 'IoU: {:.2f}'.format(iou_value), (2, 35), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image_copy, 'Conf: {:.2f}'.format(conf_pred), (2, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image_copy, 'Y1: {:.0f}'.format(pred_y1), (2, 125), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image_copy, 'X1: {:.0f}'.format(pred_x1), (2, 175), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image_copy, 'Y2: {:.0f}'.format(pred_y2), (2, 225), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image_copy, 'X2: {:.0f}'.format(pred_x2), (2, 275), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 2, cv2.LINE_AA)
        if confidence > threshold:
            cv2.rectangle(image_copy, (int(annotation[1] * ORIGINAL_IMAGE_WIDTH), int(annotation[0] * ORIGINAL_IMAGE_HEIGHT)), (int(annotation[3] * ORIGINAL_IMAGE_WIDTH), int(annotation[2] * ORIGINAL_IMAGE_HEIGHT)), (255, 0, 0), 2)
...
def play_images_from_video(video_path, model, interval=100, threshold=0.5, scaled_targets=False, save_video=False, save_path=None):
    fig, ax = plt.subplots(figsize=(5, 5))
...
def copy_image_directories_random(image_directory_list, json_list, copy_directory, num_to_copy=None):
    #num_to_copy: number of images to copy from each directory
    ...
    target_values = []
    for i in range(len(image_directory_list)):
        file_name_list = os.listdir(image_directory_list[i])
        #keep only jpg files
        file_name_list = [x for x in file_name_list if '.jpg' in x]
        if num_to_copy == None:
            random_file_names = random.sample(file_name_list, len(file_name_list))
        else:
            random_file_names = random.sample(file_name_list, num_to_copy)
        for file_name in random_file_names:
            ...
            if json_list[i] == None:
                ...
                for entry in json.load(open(image_directory_list[i] + json_list[i])):
                    if entry['image'] == file_name:
                        width = entry['annotations'][0]['coordinates']['width']
                        height = entry['annotations'][0]['coordinates']['height']
...
def get_iou_value_multi(y_true, y_pred):
    at = tf.convert_to_tensor(y_true, dtype=tf.float32)
    bt = tf.convert_to_tensor(y_pred, dtype=tf.float32)
    c = iou_metric_multi(at, bt)
    return K.get_value(c)


def write_results(file_path, model, history, train_gen, val_gen, train_results, validation_results, test_results):
    result_dict = {}
    result_dict['n_training'] = train_gen.get_length()
    result_dict['train_positive'] = train_gen.get_positive_count()
    result_dict['train_negative'] = train_gen.get_negative_count()
    result_dict['n_validation'] = val_gen.get_length()
    result_dict['val_positive'] = val_gen.get_positive_count()
    result_dict['val_negative'] = val_gen.get_negative_count()
    result_dict['is_multi'] = True
    
    result_dict['training_iou'] = train_results[-1]
    result_dict['validation_iou'] = validation_results[-1]
    result_dict['testing_iou'] = test_results[-1]
    ...
    for layer in model.get_config()['layers'][23:]:
        ...
        if layer['class_name'] == 'Dense':
            ...
            layer_dict['units'] = layer['config']['rate']
...
def append_score_data(file_path, model, train_datagen, history, early_stopper, train_results, validation_results, test_results):
    file = open(file_path, 'a')
    file.write('----------Model Data----------\n')
    file.write('Positive Samples: ' + str(train_datagen.get_positive_count()) + '\n')
    file.write('Negative Samples: ' + str(train_datagen.get_negative_count()) + '\n')
    file.write('Batch Size: ' + str(train_datagen.batch_size) + '\n')
    file.write('Learning Rate: ' + str(model.optimizer.lr.numpy()) + '\n')
    file.write('Total Epochs: ' + str(len(history.history['loss'])) + '\n')
    file.write('Patience: ' + str(early_stopper.patience) + '\n')
    file.write('ES Monitor: ' + str(early_stopper.monitor) + '\n')
    file.write('Best Epoch: ' + str(np.argmax(history.history['val_bounding_output_iou__metric']) + 1) + '\n')
    max_index = np.argmax(history.history['val_bounding_output_iou__metric'])
    file.write('Best History IOU Training: ' + str(history.history['bounding_output_iou__metric'][max_index]) + '\n')
    file.write('Best History IOU Validation: ' + str(history.history['val_bounding_output_iou__metric'][max_index]) + '\n')
    file.write('Final Training IOU: ' + str(train_results[-1]) + '\n')
    file.write('Final Validation IOU: ' + str(validation_results[-1]) + '\n')
    file.write('Final Testing IOU: ' + str(test_results[-1]) + '\n')
    file.write('Layers:\n')
    for layer in model.get_config()['layers'][23:]:
        if layer['class_name'] == 'Concatenate':
            continue
        if layer['class_name'] == 'Dense':
            file.write(' Dense: ' + str(layer['config']['units']) + ' Activation: ' + str(layer['config']['activation']) + '\n')
        elif layer['class_name'] == 'Dropout':
            file.write(' Dropout: ' + str(layer['config']['rate']) + '\n')
    file.write('------------------------------\n')
    file.close()
