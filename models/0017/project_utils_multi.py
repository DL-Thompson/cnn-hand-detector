...
def play_images_from_video(video_path, model, interval=100, threshold=0.5, scaled_targets=False):
    fig, ax = plt.subplots(figsize=(5, 5))
...
def save_images_from_video(video_path, model, save_path, interval=100, threshold=0.5, scaled_targets=False):
    cap = cv2.VideoCapture(video_path)
    animation_images = []
    while(cap.isOpened()):
        ret, frame = cap.read()
